...
int
hh_epoll_create(int size __attribute__((__unused__)))
{
    return hh_kqueue();
}

int
hh_epoll_ctl(int epfd, int op, int fd, struct epoll_event *event)
{
    /*
    ...
    if (op == EPOLL_CTL_DEL) {
        ...
        return hh_kevent(epfd, kev, changes, NULL, 0, NULL);
    }
...
static void 
hh_event_to_epoll(void **ev, struct kevent *kev)
{
...
int 
hh_epoll_wait(int epfd, struct epoll_event *events, int maxevents, int timeout)
{
    int i, ret;
    ...
    return hh_kevent_do_each(epfd, NULL, 0, events, maxevents, NULL, ff_event_to_epoll);
}
